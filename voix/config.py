...
def chaine_stt() -> list[str]:
    """Les moteurs de reconnaissance qui seront reellement utilises, dans l'ordre.

    Une seule fonction decide, et l'agent comme le tableau de bord la lisent. Sans ca, le
    panneau de demarrage affiche la configuration *souhaitee* pendant que l'agent tourne sur
    autre chose — et c'est precisement le genre d'ecart qui a fait chercher une panne pendant
    deux jours du mauvais cote.

    Une cle absente retire son moteur de la chaine plutot que de la faire echouer au premier
    mot : c'est le principe de tout ce module, degrader au lieu de tomber.
    """
    if STT_ENGINE == "azure":
        return ["azure"] if AZURE_KEY else ["local"]
    if STT_ENGINE == "deepgram":
        return ["deepgram"] if DEEPGRAM_KEY else ["local"]
    if STT_ENGINE == "local":
        return ["local"]
    chaine = []
    if AZURE_KEY:
        chaine.append("azure")
    if DEEPGRAM_KEY:
        chaine.append("deepgram")
    chaine.append("local")   # toujours en queue : il ne peut pas manquer de credit
    return chaine
